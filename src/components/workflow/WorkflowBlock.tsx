      ...
      <div className={`flex-1 ${isMobile ? 'mt-2' : ''}`}>
        ...
        <div className="mt-3">
          <div className={`flex flex-wrap items-center ${isMobile ? 'gap-1' : 'gap-3'}`}>
            ...
            {step.tools && (
              <div className={`inline-flex items-center ${isMobile ? 'text-xs px-2 py-0.5' : 'px-3 py-1'} rounded-full text-sm font-medium ${isAutomated ? 'bg-purple-100 text-purple-800' : 'bg-yellow-100 text-yellow-800'} break-all flex-shrink-0`}>
                <svg className={`${isMobile ? 'w-3 h-3' : 'w-4 h-4'} mr-1`} fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
